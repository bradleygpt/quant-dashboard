...
Uses conservative rate limiting to avoid Yahoo Finance throttling on cloud servers.
Strategy: batch price download + slow sequential .info fetch with retries.
"""
...
def fetch_universe_data(
    ...
    """
    Fetch data for all tickers. Strategy:
    1. Load existing cache and keep fresh entries
    2. Batch download prices (fast, chunked with pauses)
    3. Sequential .info fetch in small batches of 5 with delays + retries
    4. Save partial results to cache every 25 tickers (progress is never lost)
    """
    cached = _load_cache()

    # Check if cache is fresh and large enough
    if _is_cache_fresh(_cache_path(FUNDAMENTALS_CACHE_FILE)) and len(cached) > 200:
        return _filter_by_market_cap(cached, min_market_cap_b)

    # Separate fresh cached entries from stale/missing ones
    results = {}
    tickers_to_fetch = []
    ...
    if progress_callback:
        progress_callback(0.25, f"Prices downloaded for {len(all_prices)} tickers. Fetching fundamentals...")

    # ── Phase 2: Sequential .info fetch in small batches ───────────
    # Process 3 at a time with a pause between each mini-batch
    completed = 0
    batch_size = 3
    ...
    for batch_start in range(0, total, batch_size):
        ...
        # Fetch this mini-batch with light threading (3 threads)
        ...
        if progress_callback:
            ...
            progress_callback(
                min(pct, 0.95),
                f"Fundamentals: {completed}/{total} done ({len(results)} in universe)..."
            )

        # Save partial results every 25 tickers so progress is never lost
        if completed % 25 == 0:
            _save_cache(results)

        # Pause between mini-batches to stay under rate limits
