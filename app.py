...
A free, data-driven stock scoring system modeled on the Seeking Alpha Quant framework.
Five pillars: Valuation, Growth, Profitability, Momentum, EPS Revisions.
"""
...
<style>
    ...
    .metric-name { color: #aaa; font-size: 0.9em; }
...
def rating_badge(rating: str) -> str:
    ...
    return f'<span class="rating-badge" style="background:{color};">{rating}</span>'
...
def make_radar_chart(pillar_scores: dict, ticker: str) -> go.Figure:
    """Create a radar chart of pillar scores for a ticker."""
...
def make_comparison_radar(tickers_data: dict) -> go.Figure:
    """Radar chart comparing multiple tickers."""
...
with st.sidebar:
    ...
    market_cap_floor = st.slider(
        ...
        min_value=MIN_MARKET_CAP_FLOOR_B,
        max_value=MAX_MARKET_CAP_FLOOR_B,
        value=DEFAULT_MARKET_CAP_FLOOR_B,
        step=1,
    )

    st.markdown("---")
    st.markdown("### Pillar Weights")
    st.caption("Adjust to reflect your strategy emphasis. Weights are normalized automatically.")
    ...
    if total_w > 0:
        st.session_state.weights = {
            "Valuation": w_val / total_w,
            "Growth": w_gro / total_w,
            "Profitability": w_pro / total_w,
...
st.markdown('<p class="sub-header">Five-pillar scoring: Valuation · Growth · Profitability · Momentum · EPS Revisions</p>', unsafe_allow_html=True)

# ── Tabs ───────────────────────────────────────────────────────────

tab_screener, tab_watchlist, tab_detail, tab_compare = st.tabs([
    "📋 Screener", "⭐ Watchlist", "🔍 Stock Detail", "⚖️ Compare"
])
...
def load_and_score(market_cap_b: float, weights_tuple: tuple):
    """Load data and compute scores. Weights passed as tuple for caching."""
    weights = dict(zip(DEFAULT_PILLAR_WEIGHTS.keys(), weights_tuple))
    tickers = get_broad_universe(market_cap_b)

    # For demo / first-load, use S&P 500 base
    if not tickers:
        tickers = get_sp500_tickers()

    # In production on Streamlit Cloud, this would run the full batch.
    # For local dev, we'll use a subset if needed.
    ...
    scored = score_universe(raw_data, weights)
    return raw_data, scored
...
try:
    raw_data, scored_df = load_and_score(market_cap_floor, weights_tuple)
    st.session_state.raw_data = raw_data
    st.session_state.scored_df = scored_df
except Exception as e:
    st.error(f"Error loading data: {e}")
    st.info("If this is your first run, data fetching may take several minutes. Please be patient.")
    st.stop()

if scored_df is None or scored_df.empty:
    st.warning("No data loaded yet. Click 'Refresh Data' in the sidebar to fetch market data.")
    st.stop()
...
with tab_screener:
    ...
    with col_filter3:
        top_n = st.selectbox("Show Top", [25, 50, 100, 250, 500], index=1)
    ...
    if filtered.empty:
        ...
        with col_s3:
            ...
            st.metric("Avg Composite", f"{avg_score:.1f}")
        with col_s4:
        ...
        with col_det1:
            detail_ticker = st.selectbox(
                ...
                format_func=lambda x: f"{x} — {filtered.loc[x, 'shortName']}",
        ...
        # Quick preview if ticker selected
        if detail_ticker:
            detail = get_pillar_detail(detail_ticker, scored_df)
            if detail:
                ...
                st.plotly_chart(
                    make_radar_chart(pillar_scores, detail_ticker),
                    use_container_width=True,
                    key="screener_radar",
                )
                    ...
                    with st.expander(f"{pillar_name}  —  Grade: {grade}  |  Score: {score:.1f}/12"):
                        for m in pillar_data["metrics"]:
                            ...
                            with c1:
                                ...
                                st.markdown(f"{m['metric']} {direction}")
                            with c2:
...
with tab_watchlist:
    ...
    if not watchlist:
        ...
        # Watchlist is already ordered oldest-first by design
        for entry in watchlist:
            ...
            if ticker in scored_df.index:
                ...
                # Expandable detail for each watchlist item
                with st.expander(f"View {ticker} Detail"):
                    detail = get_pillar_detail(ticker, scored_df)
                    if detail:
                        pillar_scores = {p: d["pillar_score"] for p, d in detail.items()}
                        st.plotly_chart(
                            make_radar_chart(pillar_scores, ticker),
                            use_container_width=True,
                            key=f"wl_radar_{ticker}",
                        )
                        for pillar_name, pillar_data in detail.items():
                            grade = pillar_data["pillar_grade"]
                            score_val = pillar_data["pillar_score"]
                            with st.expander(f"{pillar_name}: {grade} ({score_val:.1f}/12)", expanded=False):
                                for m in pillar_data["metrics"]:
                ...
                # Ticker not in scored universe
                col1, col2, col3, col4 = st.columns([1.5, 3, 2, 0.8])
                with col1:
                    st.markdown(f"**{ticker}**")
                with col2:
                    st.caption("Not in current universe (below market cap floor?)")
                with col3:
                    st.caption(f"Added {date_added}")
                with col4:
                    if st.button("✕", key=f"wl_rm_{ticker}_oos"):
                        remove_from_watchlist(ticker)
                        st.rerun()
...
with tab_detail:
    ...
    if selected and selected in scored_df.index:
        ...
        detail = get_pillar_detail(selected, scored_df)
        ...
        with col_h4:
            ...
            st.markdown(
                f'<span style="background:{RATING_COLORS.get(rating, "#666")};padding:4px 14px;border-radius:6px;font-weight:700;color:#111;font-size:1.1em;">{rating}</span>',
                unsafe_allow_html=True,
            )
        ...
        with col_chart:
            ...
            if detail:
                pillar_scores = {p: d["pillar_score"] for p, d in detail.items()}
                st.plotly_chart(
                    make_radar_chart(pillar_scores, selected),
                    use_container_width=True,
                    key="detail_radar",
                )
        ...
        # Detailed Metric Breakdown (the click-to-expand view)
        st.markdown("---")
        st.markdown("### 📊 Full Metric Breakdown")
        st.caption("Click any pillar to expand all input metrics, values, and grades.")
            ...
            for pillar_name, pillar_data in detail.items():
                ...
                with st.expander(f"{'▸'} {pillar_name}  |  Grade: {grade}  |  Score: {score_val:.1f} / 12", expanded=False):
                    ...
                    for m in pillar_data["metrics"]:
                        ...
                        with mc3:
                            g = m["grade"]
                            g_color = GRADE_COLORS.get(g, "#666")
                            st.markdown(
                                f'<span style="background:{g_color};padding:2px 8px;border-radius:4px;font-weight:700;color:#111;">{g}</span>',
                                unsafe_allow_html=True,
                            )
                        with mc4:
                            st.markdown("↑ Higher better" if m["higher_is_better"] else "↓ Lower better")
        ...
        with col_act2:
            if st.button(f"⚖️ Add {selected} to Compare", key="detail_add_cmp"):
...
with tab_compare:
    ...
    selected_compare = st.multiselect(
        "Select tickers to compare (2-5 recommended)",
        compare_options,
        default=st.session_state.compare_tickers[:5],
        max_selections=5,
        format_func=lambda x: f"{x} — {scored_df.loc[x, 'shortName']}" if x in scored_df.index else x,
    ...
    if len(selected_compare) < 2:
        ...
        for t in selected_compare:
            if t in scored_df.index:
                ...
                for p in PILLAR_METRICS:
                    row_data[f"{p}"] = f"{r.get(f'{p}_grade', 'N/A')} ({r.get(f'{p}_score', 0):.1f})"
        ...
        if bar_data:
            ...
            fig.update_layout(
                yaxis=dict(range=[0, 12], title="Score (0-12)"),
                xaxis=dict(title=""),
                height=400,
                paper_bgcolor="rgba(0,0,0,0)",
                plot_bgcolor="rgba(0,0,0,0)",
...
st.markdown("---")
st.caption(
    "Quantitative Strategy Dashboard v1.0 · Data via yfinance · "
    "Scoring methodology inspired by Seeking Alpha Quant framework · "
